...
public abstract class SlotDisplayFeature<F extends SlotDisplayFeature<F, N, I>.Instance, N extends Node.Scoped<N, I, ?, ?>, I extends Item.Scoped<I, N>>
    ...
    public abstract class Instance extends AbstractInstance<N> {
        ...
        private int linesSetup(Map<String, KeyData> keyData, Map<NodePath, Integer> slotDepths, Locale locale, Tree<N> tree, N node, AtomicInteger longest) {
            var slots = node.value().slots();
            int size = 0;
            for (var entry : slots.entrySet()) {
                String key = entry.getKey();
                size += 1 + node.node(key)
                        .map(child -> linesSetup(keyData, slotDepths, locale, tree, child, longest))
                        .orElse(0);
        ...
        private void lines(List<Component> lines, Locale locale, Map<String, KeyData> keyData, Map<NodePath, Integer> slotDepths, int longest, Tree<N> tree, N node, Component indent, int depth) {
            var slots = node.value().slots();
            ...
            for (var key : slotOrder) {
                ...
                optChild.ifPresent(child -> lines(lines, locale, keyData, slotDepths, longest, tree, child, indent, depth + 1));
            }
        }
        ...
        private void onCreateItem(CreateItemEvent<N, I> event) {
            ...
            linesSetup(keyData, slotDepths, locale, event.tree(), event.tree().root(), longest);
            lines(lines, locale, keyData, slotDepths, longest.get(), event.tree(), event.tree().root(), indent, 0);
