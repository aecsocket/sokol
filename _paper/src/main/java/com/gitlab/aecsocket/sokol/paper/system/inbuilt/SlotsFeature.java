...
public class SlotsFeature extends AbstractFeature implements PaperFeature {
    ...
    public final class Instance extends AbstractFeature.Instance implements PaperFeature.Instance {
        ...
        @Override public SlotsFeature base() { return SlotsFeature.this; }
        ...
        protected void event(Events.CombineNodeOntoParent event) {
            ...
            parent.stats().<List<PreciseSound>>val("combine_sounds")
        ...
        protected void event(Events.InsertInto event) {
            if (!parent.isRoot())
                return;
            Location location = event.handle.getWhoClicked().getLocation();
            parent.stats().<List<PreciseSound>>val("insert_sounds")
                    .ifPresent(v -> v.forEach(s -> s.play(platform, location)));
        }
        ...
        protected void event(Events.RemoveFrom event) {
            ...
            event.node.asRoot().stats().<List<PreciseSound>>val("remove_sounds")
                    .ifPresent(v -> v.forEach(s -> s.play(platform, location)));
        }
                ...
                platform.guis()
                        .create(new SlotViewPane(platform, 9, 6, locale, root)
                                ...
                                .treeModifyCallback(node ->
                                        handle.setCurrentItem(node.system(PaperItemFeature.KEY).orElseThrow().create(locale).handle())), evt -> {
    ...
    public SlotsFeature(SokolPlugin platform, int listenerPriority, boolean slotView, boolean slotViewModification, boolean slotViewLimited, boolean combine, boolean combineLimited) {
    ...
    public static ConfigType type(SokolPlugin platform) {
        return cfg -> new SlotsFeature(platform,
