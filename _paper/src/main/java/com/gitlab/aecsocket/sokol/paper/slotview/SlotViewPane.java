...
import com.gitlab.aecsocket.sokol.paper.system.inbuilt.SlotsFeature;
import com.gitlab.aecsocket.sokol.paper.system.inbuilt.PaperItemFeature;
...
public class SlotViewPane extends Pane {
        ...
        private void event(InventoryClickEvent event) {
            ...
            if (event.getClick() == ClickType.LEFT) {
                if (child == null) {
                    ...
                    // cursorNode -> [slot]
                    // _, cursorNode
                    SlotsFeature.Events.SlotModify slotModify = new SlotsFeature.Events.SlotModify(event, parent, slot, null, cursorNode);
                    SlotsFeature.Events.InsertInto insertInto = new SlotsFeature.Events.InsertInto(event, cursorNode, parent, slot, null);
                    ...
                    PaperItemFeature.Instance itemSystem = orphanChild.system(PaperItemFeature.KEY).orElse(null);
                    if (itemSystem == null)
                        return;
                    ...
                    if (PaperUtils.empty(cursor)) {
                        // child -> [cursor]
                        // slotNode, _
                        SlotsFeature.Events.SlotModify slotModify = new SlotsFeature.Events.SlotModify(event, parent, slot, child, null);
                        SlotsFeature.Events.RemoveFrom removeFrom = new SlotsFeature.Events.RemoveFrom(event, child, parent, slot, null);
                        ...
                        if (cursor.isSimilar(nodeItem)) {
                            ...
                            // slotNode, cursorNode
                            SlotsFeature.Events.SlotModify slotModify = new SlotsFeature.Events.SlotModify(event, parent, slot, child, cursorNode);
                            SlotsFeature.Events.RemoveFrom removeFrom = new SlotsFeature.Events.RemoveFrom(event, child, parent, slot, cursorNode);
                            SlotsFeature.Events.InsertInto insertInto = new SlotsFeature.Events.InsertInto(event, cursorNode, parent, slot, child);
    ...
    private ItemStack item(@Nullable PaperTreeNode node) {
        return node == null
                ...
                : node.system(PaperItemFeature.KEY)
