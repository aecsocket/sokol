...
public abstract class SchedulerFeature<E extends ItemTreeEvent.Hold> extends AbstractFeature {
    /** The system ID. */
    public static final String ID = "scheduler";
    public static final Key<SchedulerFeature<?>.Instance> KEY = new Key<SchedulerFeature<?>.Instance>(ID, new TypeToken<SchedulerFeature<?>.Instance>() {});

    public static final long EXPIRE_THRESHOLD = 100;

    public interface GlobalScheduler<E extends ItemTreeEvent.Hold> {
        <N extends TreeNode.Scoped<N, ?, ?, ?, Y>, Y extends Feature.Instance> int schedule(Y system, long delay, SystemTask<E, N, Y> task);
    ...
    private static final class GlobalSchedulerImpl<E extends ItemTreeEvent.Hold> implements GlobalScheduler<E> {
        private record Task<E extends ItemTreeEvent.Hold, N extends TreeNode.Scoped<N, ?, ?, ?, Y>, Y extends Feature.Instance>(
                String[] path,
                String systemId,
                long runAt,
        ...
        @Override
        public <N extends TreeNode.Scoped<N, ?, ?, ?, Y>, Y extends Feature.Instance> int schedule(Y system, long delay, SystemTask<E, N, Y> task) {
        ...
        private <N extends TreeNode.Scoped<N, ?, ?, ?, Y>, Y extends Feature.Instance> boolean run0(Task<E, N, Y> task, E event, int id) {
    ...
    public record TaskContext<N extends TreeNode.Scoped<N, ?, ?, ?, Y>, Y extends Feature.Instance>(N root) {
    ...
    public interface SystemTask<E extends ItemTreeEvent.Hold, N extends TreeNode.Scoped<N, ?, ?, ?, Y>, Y extends Feature.Instance> {
        void run(Y self, E event, TaskContext<N, Y> ctx);
    }

    protected final GlobalScheduler<E> scheduler;

    public SchedulerFeature(GlobalScheduler<E> scheduler, int listenerPriority) {
    ...
    /**
     * See {@link SchedulerFeature}.
     */
    public abstract class Instance extends AbstractFeature.Instance implements Availability {
        ...
        @Override public abstract SchedulerFeature<E> base();
