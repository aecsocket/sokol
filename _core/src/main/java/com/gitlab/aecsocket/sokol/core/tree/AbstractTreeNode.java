 ...
 */
public abstract class AbstractTreeNode<N extends AbstractTreeNode<N, C, S, B, Y>, C extends Component.Scoped<C, S, B>, S extends Slot, B extends Feature, Y extends Feature.Instance>
    ...
    @Override
    public void child(String key, @Nullable TreeNode value) throws IllegalArgumentException {
        S slot = this.value.slot(key)
        ...
        if (value == null)
            children.remove(key);
        else if (slot.compatible(this, value)) {
            @SuppressWarnings("unchecked")
            N nValue = (N) value;
            children.put(key, nValue);
            value.parent(this, slot);
        } else
            throw new IllegalArgumentException("Cannot add child [" + value.value().id() + "] into slot [" + key + "] as it is not compatible");
    }
    ...
    @SuppressWarnings("unchecked")
    @Override public <T extends Feature.Instance> Optional<T> system(Feature.Key<T> key) { return Optional.ofNullable((T) systems.get(key.id())); }
    @Override public Optional<? extends Feature.Instance> system(String id) { return Optional.ofNullable(systems.get(id)); }
    @Override
    ...
    @Override
    public <T extends Feature.Instance> Optional<T> system(Class<T> type) {
        for (Feature.Instance system : systems.values()) {
    ...
    @Override
    public void parent(@Nullable TreeNode parent, @Nullable Slot slot) {
    }
